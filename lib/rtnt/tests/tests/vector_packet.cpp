...
TEST_F(NetworkTest, vector_packet)
{
    ...
    server->getPacketDispatcher().bind<Example>([&](const auto&, const Example& pkt) { receivedData1 = pkt.data; receivedData2 = pkt.data2; });
    ...
    ASSERT_TRUE(waitFor([&]() { return !receivedData1.empty() && !receivedData2.empty(); })) << "Server has received no EXAMPLE packet.";
